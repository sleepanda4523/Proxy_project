    ...
    def if_have_cert(self):
        ...
        with self.lock:  # = lock.acquire() ~ lock.release()
            if not os.path.isfile(cert_path):  # 만약 연결할 서버의 인증서가 없다면
                ...
                # print(etime)
                p1 = Popen(
                    ["openssl", "req", "-new", "-key", self.certkey, "-subj", f"/CN={hostname}", "-out", "server.csr"],
                    stdout=PIPE)
                os.system(
                    f'openssl x509 -req -days 3650 -in server.csr -CA {self.cacert} -CAkey {self.cakey}  -CAcreateserial -out {cert_path}')

        # w = f"{self.protocol_version} 200 Connect OK\r\n"
        # self.wfile.write(str.encode(w))
        self.send_response(200, 'Connection Established')
        print("200 Connection Established")
        self.end_headers()
